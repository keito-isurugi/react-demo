...
export const Day = () => {
  ...
  const onDragEnd = (result) => {
      ...
      if(source.droppableId !== destination.droppableId) {
        ...
        for(let i = 0; i < data.length; i++) {
          if((data[i].cols.findIndex((e) => e.id === source.droppableId)) !== -1){
            sourceDateIndex = i;
            sourceWorksIndex = data[i].cols.findIndex((e) => e.id === source.droppableId);
        ...
        for(let i = 0; i < data.length; i++) {
          if((data[i].cols.findIndex((e) => e.id === destination.droppableId)) !== -1){
            ...
            destinationWorksIndex = data[i].cols.findIndex((e) => e.id === destination.droppableId);
        ...
        if(sourceWorks !== undefined){
          ...
          data[destinationDateIndex].cols[destinationWorksIndex].tasks = destinationTask;
          setData(data);
        }
      } else {
        let sourceDateIndex;
    ...
    if(destination !== null) {
      // リスト→カラム
      // 移動元
      const sourceDateStaffsIndex = (data.findIndex((e) => e.mansId === source.droppableId))
      ...
      // 移動先
      for(let i = 0; i < data.length; i++) {
        if((data[i].cols.findIndex((e) => e.manId === destination.droppableId)) !== -1){
          // 移動先の配列の親配列番号取得
          destinationDateManIndex = i;
          // 移動先の配列の親配列を取得
          destinationStaffsIndex = data[i].cols.findIndex((e) => e.manId === destination.droppableId);
          // 移動先の配列を取得
      ...
      if(sourceStaffsIndex !== undefined) {
        ...
        if(sourceStaffsIndex.id === destinationDate.id) {
          ...
          data[destinationDateManIndex].cols[destinationStaffsIndex].man = destinationStaff;
          setData(data)
        }
      }
      ...
      let sourceStaff;
      for(let i = 0; i <= data.length; i++) {
        if((data[i].cols.findIndex((e) => e.manId === source.droppableId)) !== -1){
          sourceDateIndex = i;
          sourceStaffIndex = data[i].cols.findIndex((e) => e.manId === source.droppableId);
        ...
        if(data[sourceDateIndex].id === destinationDate.id) {
          ...
          for(let i = 0; i <= data.length; i++) {
            if((data[i].cols.findIndex((e) => e.manId === destination.droppableId)) !== -1){
              ...
              destinationStaffColIndex = data[i].cols.findIndex((e) => e.manId === destination.droppableId);
          ...
          data[destinationStaffDateIndex].cols[destinationStaffColIndex].man = destinationTask;
          setData(data);
        } 
      } else {
        // カラム→リスト
        const destinationStaffsDateIndex = (data.findIndex((e) => e.mansId === destination.droppableId))
        const destinationStaffsDate = data[destinationStaffsDateIndex];
        if(data[sourceDateIndex].id === destinationStaffsDate.id){
          const sourceStaffCp = [...sourceStaff.man];
          const [removed] = sourceStaffCp.splice(source.index, 1);
          // 移動先の日付のindex、案件の配列を取得
          const destinationDateIndex = (data.findIndex((e) => e.mansId === destination.droppableId))
          if(data[destinationDateIndex] !== undefined) {
            ...
            data[destinationDateIndex].cols[sourceStaffIndex].man = sourceStaffCp;
  ...
  return (
    <>
      <Text fontSize="3xl" fontWeight={"bold"}>直近工事予定</Text>
      <DragDropContext onDragEnd={onDragEnd}>
        <Box display="flex" w="100%" textAlign="center">
          {data.map((day, index) => 
          <Grid templateRows='repeat(1, 1fr)' templateColumns='repeat(9, 1fr)' h="100%" w={"100%"} border='1px' borderColor='red' key={day.id}>
            <GridItem rowSpan={1} colSpan={1} border='1px' borderColor='green'>
              <Box border='1px' borderColor='black.200' h={"100px"}>{day.date}</Box>
            ...
            {day.cols.map((col, index) => 
              <GridItem border='1px' borderColor='black.200' key={col.id}>
                ...
                <Droppable key={col.manId} droppableId={col.manId} type="STAFFS">
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} >
                      <Box h={"70px"}>
                        {col.man.map((m, index) => (
            ...
            <GridItem rowSpan={1} colSpan={1} border='1px' borderColor='green'>
              <Box border='1px' borderColor='black.200' h={"100px"}>空白</Box>
            </GridItem>
          </Grid>
          )}
        </Box>
