...
export const DndGrid = () => {
  ...
  return (
    <>
      <Text fontSize="3xl" fontWeight={"bold"}>週間工事予定</Text>
      <DragDropContext onDragEnd={onDragEnd}>
        <Text fontSize="xl" fontWeight={"bold"} p={[0,0,3,2]}>2022年6月</Text>
        <Box display="flex" w="50%" textAlign="center">
          {data.map((day, index) => 
          <Grid templateColumns='repeat(2, 1fr)' h="200px" w={"50%"} border='1px' borderColor='red' key={day.id}>
            <GridItem colSpan={7} border='1px' borderColor='green'>
              {day.date}
            </GridItem>
            {day.cols.map((col, index) => 
              <GridItem border='1px' borderColor='black.200' key={col.id}>
                <Box border='1px' borderColor='black.200'>{col.car}</Box>
                <Box border='1px' borderColor='black.200'>{col.man}</Box>
                <Droppable key={col.id} droppableId={col.id}>
                  {(provided) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} >
                      <Box border='1px' borderColor='black.200' minH={"100px"}>
